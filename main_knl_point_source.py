...
class ModelingProcess(Process):
    ...
    def run(self):
        while not self.parameters.empty():
            ...
            source = Source(
                coordinates=(detector.size[0]/2, collimator.coordinates[1] + distance, detector.size[1]/2),
                activity=70*10**6,
            ...
            size = np.asarray((detector.size[0], source.coordinates[1], detector.size[1]))
            space = Space(size, 0)
            ...
            modeling = Modeling(
                ...
                time_step=0.01,
                subject=detector,
                file_name='point source ' + parameters['Name'] + f' {distance} cm' + '.hdf'
                )
...
def generateParameters():
    ...
    startTime = np.round(np.linspace(0, totalTime, (cores_number//(len(distances)*len(names))) + 1), 2)[:-1]
